...
class TestPerson:
    ...
    def test_job_not_in_list(self):
        ...
        captured_out = io.StringIO()
        sys.stdout = captured_out
        # Use a valid name to prevent the name validation message from being printed
        Person(name="Valid Name", job="Benevolent dictator for life")
        sys.stdout = sys.__stdout__
        # Assert that only the job validation message is printed
        assert(captured_out.getvalue().strip() == "Job must be in list of approved jobs.")
